<script lang="ts">
	...
	import { any } from 'zod';
	import Input from '../../masterData/Input.svelte';
	...
	let {
		data = null,
		name = '',
		actionEdit = $bindable<Act>(),
		actionDel = $bindable<Act>(),
		tipe = '',
		del = false,
		edit = false,
		id = null
...
{#if edit}
	...
	{#if tipe === 'anggota'}
		...
		>
			<KerajaanPopup {data} bind:value={edit} bind:error type="Ubah" dataGelar={data.gelar}
			></KerajaanPopup>
		</form>
	{/if} -->
{/if}
