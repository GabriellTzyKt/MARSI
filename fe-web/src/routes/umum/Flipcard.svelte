<script lang="ts">
	let { gambar, kerajaan, judul, lokasi, isi, data } = $props();
...
<div class="perspective group h-[520px] w-[360px] cursor-pointer rounded-full bg-transparent">
	...
	>
		<div class="backface-hidden absolute h-full w-full">
			<div
				class="text-md absolute bottom-0 flex h-fit w-full flex-col p-2 font-semibold rounded-b-lg text-white border-2 bg-red-500 px-2"
			>
				...
				<p class="inline-flex items-center gap-2 text-md">
					<span class="bxs--map"></span>
					{lokasi}
				</p>
			</div>
			<img src={gambar} alt="duar" class="h-full w-full rounded-lg" />
		</div>
		...
		>
			<div class="flex flex-col">
				...
				<div class="flex w-full absolute bottom-0 right-4 justify-center lg:justify-end">
					<a
						href="/umum/daftarkerajaan/{data.id_kerajaan}"
