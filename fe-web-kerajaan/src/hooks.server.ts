...
export const handle = async ({ event, resolve}) => {
    ...
    const publicRoutes = ['/login', '/signup','/kelompok', '/beranda',"/acara","/situs","/aset", "/sabdi"];
    ...
    if (auth) {
        ...
        if (event.url.pathname.startsWith("/abdi/sekretariat")) {
            if (!auth?.id_admin || auth.jenis_admin === 0 || auth.status_admin_aktif === 0 || auth.jenis_admin !=="Super Admin") {
                throw redirect(302, '/beranda');
            }
        }
        ...
        if (event.url.pathname.startsWith("/abdi/dashboard/situs")) {
            if (!auth?.id_admin || auth.jenis_admin === 0 || auth.status_admin_aktif === 0 || auth.jenis_admin !== "Admin Situs") {
                throw redirect(302, '/beranda');
            }
        }
