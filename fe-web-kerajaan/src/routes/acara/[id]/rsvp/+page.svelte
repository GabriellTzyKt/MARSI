<script lang="ts">
	...
	let { data } = $props()
	let datauser : any = $state(data.allUsers)
	...
	>
		...
		<div class="grid grid-cols-1 gap-8 px-10 py-10 md:grid-cols-2">
			<div>
				...
				<div class="mt-4 flex flex-col">
					...
					{#if error?.namalengkap}
						<p class="text-left text-red-500">- {error.namalengkap}</p>
					{/if}
				</div>
			...
			<div>
				<div class="mb-2 flex items-center justify-between">
					...
					{#if show}
						<div class="flex items-center">
							<button
								type="button"
								class="flex w-8 h-8 items-center justify-center rounded-lg bg-red-600 text-white mr-2"
								onclick={decrease}
							>
							 -
							</button>
							...
							<button
								type="button"
								class="flex w-8 h-8 items-center justify-center rounded-lg bg-green-700 text-white ml-2"
								onclick={increase}
							>
							 +
							</button>
				...
				{#if show}
					<div class="mb-4">
						<h3 class="font-medium mb-2">Daftar Tamu Tambahan</h3>
						{#if error?._tamu}
							<p class="text-left text-red-500 mb-2">- {error._tamu}</p>
						{/if}
					</div>
					
					{#each Array(number) as _, i}
						<Rsvp index={i} errors={error}></Rsvp>
					{/each}
				{/if}
