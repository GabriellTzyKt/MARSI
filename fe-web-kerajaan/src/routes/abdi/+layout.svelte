<script lang="ts">
	...
	const pageTitle = $derived.by(() => {
		...
		// test baru dengan ada id
			...
			page.route.id === '/abdi/dashboard/situs/beranda/[id]/detail/daftarkomunitas/[idkom]'
		) {
			return 'Daftar Komunitas di Situs';
		} else if (
			page.route.id ===
			'/abdi/dashboard/situs/beranda/[id]/detail/daftaracara/detail/[idacarasitus]'
		) {
			return 'Detail Acara Situs';
			...
			page.route.id ===
			'/abdi/dashboard/situs/beranda/[id]/detail/daftaracara/laporan/[idacarasitus]'
		) {
			return 'Detail Laporan Acara Situs';
		} else if (
...
<header class="w-full lg:flex lg:h-full lg:min-w-full">
	...
	<div class="width_head2 hidden items-center justify-between lg:flex lg:w-[83.3%]">
		...
		<div
			class="relative mr-5 flex w-56 items-center justify-between rounded-t-lg border-2 border-blue-600 py-2"
		>
			...
			{#if showLogout}
				<div
					class="w-55 absolute right-0 top-14 z-50 flex flex-col items-center rounded-b-lg border bg-white p-4 shadow-lg"
