...
export const load: PageServerLoad = async ({ params, cookies }) => {
    try {
        ...
        if (komunitasData.profile) {
            let profilePath = komunitasData.profile.split(",")[0]
            try {
                const filePathResponse = await fetch(`${env.URL_KERAJAAN}/doc/${profilePath}`);
                if (filePathResponse.ok) {
                    let filePathData = await filePathResponse.json();
                    console.log("File path data:", filePathData);
                    let filePath = filePathData.file_dokumentasi;
                    if (filePath) {
                        profileUrl = {
                            path: filePath,
     ...
     let fotoOrganisasiDetails = [];
          const dokumentasiValue = komunitasData.foto_komunitas?.toString() || '';
...
export const actions: Actions = {
    updateFoto: async ({ request, params, cookies }) => {
        ...
        console.log(data)
        const existingDocResponse = await fetch(`${env.URL_KERAJAAN}/komunitas/${params.id}`);    
        const existingDoc = existingDocResponse.ok ? await existingDocResponse.json() : { foto_komunitas: '' };  
        ...
        if (newFiles.length > 0) {              
            try {      // Upload each file individually to ensure all are processed
                for (const file of newFiles) {         
                    ...
                    singleFileFormData.append('nama_komunitas', existingDoc.nama_komunitas as string);   
                    singleFileFormData.append('foto_komunitas', file);                               
                    console.log(`Uploading file: ${file.name}`);         
                    const uploadResponse = await fetch(`${env.URL_KERAJAAN}/file/foto_komunitas`, {                          
                        method: 'POST',                 
        ...
        const payload = {     
            id_komunitas: Number(existingDoc.id_komunitas),     
            penanggung_jawab: Number(existingDoc.penanggung_jawab),     
            foto_komunitas: finalDokumentasi,
            profile: existingDoc.profile,                            
            lokasi: existingDoc.lokasi,                            
            nama_komunitas: existingDoc.nama_komunitas,     
            deskripsi: existingDoc.deskripsi_komunitas, 
            pelindung: existingDoc.pelindung, 
            pembina: existingDoc.pembina, 
            alamat: existingDoc.alamat, 
            tanggal_berdiri: formatDatetoUI(existingDoc.tanggal_berdiri),              
