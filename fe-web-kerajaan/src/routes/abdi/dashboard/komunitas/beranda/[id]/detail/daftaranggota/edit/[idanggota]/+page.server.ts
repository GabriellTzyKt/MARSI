...
export const load: PageServerLoad = async ({ params, cookies }) => {
    ...
    try {
        ...
        let anggotaRes = await fetch(`${env.URL_KERAJAAN}/komunitas/anggota/${params.id}?limit=1000`, {
            method:"GET"
        })
    ...
    edit: async ({ request, cookies,params }) => {
       ...
       let  form = {
        ...
        try {
            ...
            let anggotaEditRes = await fetch(`${env.URL_KERAJAAN}/komunitas/anggota`, {
                method: "PUT",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${token?.token}`
