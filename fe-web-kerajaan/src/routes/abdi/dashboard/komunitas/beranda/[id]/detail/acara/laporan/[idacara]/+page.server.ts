...
export const load: PageServerLoad = async ({params, cookies}) => {
    try {
        ...
        let resSitus = await fetch(`${env.URL_KERAJAAN}/organisasi`);
        let res = await fetch(`${env.URL_KERAJAAN}/acara/detail/${params.idacara}`);
        let resUndangan = await fetch(`${env.URL_KERAJAAN}/undangan/${params.idacara}`);
        let resPanit = await fetch(`${env.URL_KERAJAAN}/acara/panitia/${params.idacara}`);
        console.log(res)
        ...
        for (const docId of docIds) {
            ...
            let resLpj = await fetch(`${env.URL_KERAJAAN}/lpj/${params.idacara}`);
            if (!resLpj.ok) {
                console.error(`Failed to fetch lpj/${params.idacara}: ${resLpj.status}`);
...
export const actions: Actions = {
    updateLaporan: async ({ request, params }) => {
        ...
        let lpjRes = await fetch(`${env.URL_KERAJAAN}/lpj/${params.idacara}`, {
            method: "GET",
        });
        let lpjData = await lpjRes.json();
        console.log(lpjData)
        if (!lpjRes) {
            console.error(`Failed to fetch lpj/${adc}: ${lpjRes.status}`);
        ...
        if (!id_lpj) {
            ...
            lpj.append("id_acara", (params.idacara as string))
            ...
            if (!lpjRes) {
                console.error(`Failed to fetch lpj/${params.idacara}: ${lpjRes.status}`);
                // return fail(404, { error: "Lpj tidak ditemukan" });
            }
            let lpjjres = await fetch(`${env.URL_KERAJAAN}/lpj/${params.idacara}`, {
                method: "GET",
            });
        ...
        try {
            ...
            const existingDocResponse = await fetch(`${env.URL_KERAJAAN}/acara/detail/${params.idacara}`);
        ...
        // let gambarf;
            ...
            const payload = {
                id_acara: Number(params.idacara),              
