...
export const load: PageServerLoad = async ({params, cookies}) => {
    try {
        let id_acara = params.idacara
        let id = params.id
        let cook = JSON.parse(cookies.get("userSession") as string)
        let resSitus = await fetch(`${env.URL_KERAJAAN}/organisasi`);
        let res = await fetch(`${env.URL_KERAJAAN}/acara/detail/${id_acara}`);
        let resUndangan = await fetch(`${env.URL_KERAJAAN}/undangan/${id_acara}`);
        let resPanit = await fetch(`${env.URL_KERAJAAN}/acara/panitia/${id_acara}`);
        console.log(res)
        if (res.ok && resUndangan.ok && resPanit.ok && resSitus.ok) {
            ...
            let panitWithUser = await Promise.all(panit.map(async (item : any) => { 
                try {
            ...
            console.log(panit)
                ...
                return { data: formattedData, undangan: undanganWithUser, panit: panitWithUser, situs: situs, id, id_acara };
