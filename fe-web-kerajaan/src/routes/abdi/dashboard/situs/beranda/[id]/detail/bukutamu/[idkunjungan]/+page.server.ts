...
export const load: PageServerLoad = async ({params, cookies}) => {
    try {
        let res = await fetch(`${env.URL_KERAJAAN}/situs/kunjungan/${params.idkunjungan}`);
        ...
        if(!userRes.ok) {
            throw new Error(`HTTP Error! Users Status: ${userRes.status}`)
        }
...
export const actions: Actions = {
    tambah: async ({ request, params }) => {
        ...
        const entryPromises = Array.from(entryIds).map(async id => {
            const id_situs = parseInt(params.idkunjungan)
