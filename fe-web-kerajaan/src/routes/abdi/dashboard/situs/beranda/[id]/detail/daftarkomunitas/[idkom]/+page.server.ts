     ...
     try {
         ...
         console.log("Komunitas data:", komunitasData);
          let [pelindungRes, pembinaRes, penanggungJawabRes] = await Promise.all([
            ...
            if (!pelindungRes.ok || !pembinaRes.ok || !penanggungJawabRes.ok) {
              
            }
            
         // Add pelindung, pembina, and penanggung_jawab to komunitasData
         let pelindung = await pelindungRes.json();
         let pembina = await pembinaRes.json();
         let penanggung_jawab = await penanggungJawabRes.json();
         komunitasData = {
             ...komunitasData,
             pelindung : pelindung.nama_lengkap || 'Tidak ada pelindung',
             pembina: pembina.nama_lengkap || 'Tidak ada pembina',
             penanggung_jawab: penanggung_jawab.nama_lengkap || 'Tidak ada penanggung jawab'
         };
