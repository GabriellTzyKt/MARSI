...
export const load: PageServerLoad = async ({params, cookies}) => {
    try {
        ...
        let resSitus = await fetch(`${env.URL_KERAJAAN}/situs?limit=1000`);
        let res = await fetch(`${env.URL_KERAJAAN}/acara/detail/${params.idacarasitus}`);
        let resUndangan = await fetch(`${env.URL_KERAJAAN}/undangan/${params.idacarasitus}`);
        let resPanit = await fetch(`${env.URL_KERAJAAN}/acara/panitia/${params.idacarasitus}`);
        console.log(res)
        ...
        for (const docId of docIds) {
            ...
            let resLpj = await fetch(`${env.URL_KERAJAAN}/lpj/${params.idacarasitus}`);
            if (!resLpj.ok) {
...
export const actions: Actions = {
    updateLaporan: async ({ request, params }) => {
        ...
        let lpjRes = await fetch(`${env.URL_KERAJAAN}/lpj/${params.idacarasitus}`, {
            method: "GET",
        });
        ...
        if (!id_lpj) {
            ...
            lpj.append("id_acara", (params.idacarasitus as string))
            ...
            if (!lpjRes) {
                console.error(`Failed to fetch lpj/${params.idacarasitus}: ${lpjRes.status}`);
                // return fail(404, { error: "Lpj tidak ditemukan" });
            }
            let lpjjres = await fetch(`${env.URL_KERAJAAN}/lpj/${params.idacarasitus}`, {
                method: "GET",
            });
        ...
        try {
            ...
            const existingDocResponse = await fetch(`${env.URL_KERAJAAN}/acara/detail/${params.idacarasitus}`);
        ...
        // let gambarf;
            ...
            const payload = {
                id_acara: Number(params.idacarasitus),              
