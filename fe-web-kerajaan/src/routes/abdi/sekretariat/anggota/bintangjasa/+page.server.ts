...
import { AbdiListSchema } from "./BJScheme";
...
export const load: PageServerLoad = async () => {
    try {
        ...
        let [anggotaData, penghargaanData, acaraData] = await Promise.all([
            anggotaRes.json().then((data) => data.filter((item: any) => item.deleted_at === '0001-01-01T00:00:00Z' || !item.deleted_at)),
            penghargaanRes.json().then((data) => data.filter((item: any) => item.deleted_at === '0001-01-01T00:00:00Z' || !item.deleted_at)),
    ...
    lantikAbdi: async ({ request }) => {
        ...
        for (const [key, value] of formData.entries()) {
            ...
            if (match) {
                ...
                if (field === 'bukti_abdi' && value instanceof File) {
                    ...
                    // Check if file is an image (optional, adjust as needed)
        ...
        if (!validationResult.success || fileErrors.length > 0) {
            ...
            const serializableData = abdiList.map(item => {
                ...
                if (serializable.bukti_bintang_jasa instanceof File) {
                    serializable.bukti_bintang_jasa_name = serializable.bukti_bintang_jasa.name;
                    delete serializable.bukti_bintang_jasa;
                }
        ...
        try {
            ...
            for (const abdi of abdiList) {
                ...
                form.append('id_penghargaan', abdi.penghargaan_id || '2')
                form.append('id_anggota', abdi.abdi_id)
                form.append('dokumentasi', abdi.bukti_bintang_jasa)
                form.append('acara_pemberian', abdi.acara_id)
                form.append('pemberi_penghargaan', abdi.pemberi_id)
                form.append('tanggal_penerimaan', tanggal())
                form.append('keterangan', abdi.keterangan || '')
                console.log('yang akan di submit : ', form)
              
                const res = await fetch(`${env.URL_KERAJAAN}/anggota/penghargaan`, {
                    method: "POST",
                    body: form,
                });
                if (!res.ok) {
