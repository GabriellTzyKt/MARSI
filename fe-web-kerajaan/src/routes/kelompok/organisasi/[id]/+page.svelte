<script lang="ts">
	...
	const tanggal_berdiri = organisasi?.tanggal_berdiri
		? new Date(organisasi.tanggal_berdiri).toLocaleDateString('id-ID')
		: '-';
	const alamat = organisasi?.alamat || 'Alamat tidak tersedia';
	const deskripsi = organisasi?.deskripsi_organisasi || 'Tidak ada deskripsi';
	const id = organisasi?.id_organisasi || '';
	// Get penanggung jawab name    const penanggung_jawab = data?.penanggungJawab?.nama_lengkap || organisasi?.penanggung_jawab || '-';
	// Handle image URLs safely
	const imageUrls = data?.imageUrls || [];
	const gambar1 = imageUrls[0] || 'https://picsum.photos/200/300'; // Use placeholder if no image
	const gambar2 = imageUrls[1] || gambar1; // Fallback to first image or placeholder    const gambar3 = imageUrls[2] || gambar1;
				...
				<div>
					<div class="mt-5 flex items-center">
						...
						<div class="text-md flex-col text-start">
							<p class="ml-4">Penanggung Jawab Organisasi : {penanggung_jawab}</p>
						</div>
					</div>
					<div class="mt-5 flex items-center">
					...
					<!-- <div class="mt-8 flex items-center">
						...
						>
							<a href="/kelompok/organisasi/{idAktif}/anggota">Lihat Anggota</a></button
						>
					</div> -->
				</div>
			</div>
...
<section class="flex h-fit min-w-full overflow-hidden"><Footer></Footer></section>
