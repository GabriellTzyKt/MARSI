...
<div class="mt-4 flex w-full flex-col gap-y-2 md:gap-y-0 lg:flex-row lg:justify-between">
	...
	<div class="flex flex-col items-center gap-3 md:flex-row">
		<div class="w-full">
			<button
				class="hover:bg-badran-bt mx-2 w-full rounded-lg bg-white px-3 py-2 hover:text-white md:mx-0 md:w-auto"
				disabled={currPage === 1}
				onclick={() => {
					currPage--;
				}}>Previous</button
		...
		<div class="flex w-full justify-center gap-2 md:justify-start">
			{#if total_pages <= 5}
				{#each Array(total_pages) as _, i}
					<button
						class="hover:bg-badran-bt rounded-lg p-3 hover:text-white"
						class:bg-badran-bt={currPage === i + 1}
			...
			{:else}
				...
				{#each Array(Math.min(3, total_pages - 2)) as _, i}
					{#if currPage > 2 && currPage < total_pages - 1}
						...
						{#if currPage - 1 + i > 1 && currPage - 1 + i < total_pages}
							<button
								class="hover:bg-badran-bt rounded-lg p-3 hover:text-white"
								class:bg-badran-bt={currPage === currPage - 1 + i}
					...
					{:else if currPage <= 2}
						...
						{#if i + 2 < total_pages}
							<button
								class="hover:bg-badran-bt rounded-lg p-3 hover:text-white"
								class:bg-badran-bt={currPage === i + 2}
					...
					{:else}
						...
						{#if total_pages - 3 + i > 1}
							<button
								class="hover:bg-badran-bt rounded-lg p-3 hover:text-white"
								class:bg-badran-bt={currPage === total_pages - 3 + i}
				...
				<button
					class="hover:bg-badran-bt rounded-lg p-3 hover:text-white"
					class:bg-badran-bt={currPage === total_pages}
